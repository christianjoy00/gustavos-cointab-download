...
let state={tablets:[],allTablets:[],hiddenTablets:[],licenses:[],history:[],licenseFilter:'ALL',selectedApps:new Set(),updateSelectedApps:new Set(),uninstallSelectedApps:new Set(),dashboardUrl:location.href};
...
function locationUninstallAppsModal(location){
  ...
  const render=()=>{
    ...
    $('#location-uninstall-list').innerHTML=visible.map(a=>`<label class="app-item">
      ...
      <span class="app-metrics"><strong class="app-size">${esc(formatAppSize(a.sizeBytes))}</strong><small class="app-usage">USAGE: ${esc((Number(a.usageMs)||0)>0?formatAppUsage(a.usageMs):((Number(a.lastUsedAt)||0)>0?'SEEN':'NEVER'))}</small></span>
...
function removedTabletsPanel(){
  if(!state.hiddenTablets.length)return '';
  return `<section class="removed-tablets-panel"><div class="removed-tablets-head"><div><h2>REMOVED TABLETS</h2><p>Hidden from this dashboard only. Licenses remain assigned.</p></div><span>${state.hiddenTablets.length} HIDDEN</span></div><div class="removed-tablets-list">${state.hiddenTablets.sort(tabletNaturalCompare).map(removedTabletRow).join('')}</div></section>`;
}
function bindRemovedTabletButtons(){
  $$('[data-restore-tablet]').forEach(button=>button.onclick=async()=>{
    const key=String(button.dataset.restoreTablet||'');
...
function settingsModal(id){
  ...
  <section class="admin-section"><h3>Connection & Mode</h3><div class="form-grid admin-grid">
    ...
    <label>Cleanup Delay (minutes)<input id="m-cleanup" type="number" min="0" max="1440" value="${Number(s.cleanup??10)}"></label><label class="check"><input id="m-clear-facebook" type="checkbox" ${s.clearFacebookAfterRental!==false?'checked':''}> Clear Facebook / Messenger After Rental</label><label class="check"><input id="m-clear-google" type="checkbox" ${s.clearGoogleAccountsAfterRental!==false?'checked':''}> Clear Gmail / Customer Google Accounts After Rental</label>
    <label>Preview Refresh (seconds)<input id="m-preview" type="number" min="1" max="120" value="${Number(s.screenMonitorRemoteInterval??1)}"></label>
  </div></section>
  <section class="admin-section"><h3>Automatic Maintenance</h3><p>Automatic updates only run after the tablet has remained on the CoinTab lock screen for the selected number of hours. Restart also waits for the lock screen.</p><div class="form-grid admin-grid">
    <label class="check"><input id="m-auto-update" type="checkbox" ${s.autoUpdateEnabled?'checked':''}> Automatically Update Launcher From GitHub</label>
    <label>Locked Hours Before Update<input id="m-auto-update-hours" type="number" min="1" max="168" value="${Number(s.autoUpdateLockHours??3)}"></label>
    <label>Update Hour (-1 any time, 0-23 PH)<input id="m-auto-update-hour" type="number" min="-1" max="23" value="${Number(s.autoUpdateHour??-1)}"></label>
    <label class="check"><input id="m-auto-restart" type="checkbox" ${s.autoRestartEnabled?'checked':''}> Automatic Daily Restart (Lock Screen Only)</label>
    <label>Restart Hour (0-23 PH)<input id="m-auto-restart-hour" type="number" min="0" max="23" value="${Number(s.autoRestartHour??4)}"></label>
  </div></section>
  <section class="admin-section"><h3>Appearance & Themes</h3><p>Configure the same live Launcher brand and lock-screen text used on the tablet. Defaults: 85 / 80 / CENTER (293), 3D Glow + Pulse, Serif Bold, 3000 ms.</p><div class="form-grid admin-grid">
    <label>Brand Name<input id="m-brand" value="${esc(s.launcherBrandName||"GUSTAVO'S COINTAB")}"></label><label>Lock Subtitle<input id="m-lock-subtitle" value="${esc(s.lockBrandSubtitle||'INSERT COINS')}"></label>
    <label>Launcher Theme<select id="m-theme">${themes.map((name,i)=>`<option value="${i}">${name}</option>`).join('')}</select></label><label>Animation Style<select id="m-lock-style">${['3D GLOW + PULSE','NEON FLASH','ARCADE FLASH','BOUNCE','WAVE','HEARTBEAT','FLOAT','SWING','SHAKE','ZOOM POP','BREATHING','SLIDE LEFT-RIGHT','SLIDE UP-DOWN','TILT 3D','FLICKER','SOFT STROBE','RUBBER','ORBIT','PIXEL JUMP','COIN PULSE'].map(x=>`<option value="${x}">${x}</option>`).join('')}</select></label><label>Lock Text Font<select id="m-lock-font">${['SERIF BOLD','ARCADE MONO','SANS BLACK','SANS CONDENSED','ROUNDED BOLD','MONO BOLD','MONO REGULAR','MONO ITALIC','MONO BOLD ITALIC','SERIF REGULAR','SERIF ITALIC','SERIF BOLD ITALIC','SANS REGULAR','SANS BOLD','SANS ITALIC','SANS BOLD ITALIC','SANS LIGHT','SANS THIN','SANS MEDIUM','SANS BLACK ITALIC','CONDENSED REGULAR','CONDENSED BOLD','CONDENSED ITALIC','CONDENSED BOLD ITALIC','CONDENSED LIGHT','CONDENSED LIGHT ITALIC','ROBOTO REGULAR','ROBOTO MEDIUM','ROBOTO BOLD','ROBOTO BLACK','ROBOTO LIGHT','ROBOTO THIN','CURSIVE REGULAR','CURSIVE BOLD','CASUAL REGULAR','CASUAL BOLD','SERIF DISPLAY','SERIF DISPLAY BOLD','SANS SMALL CAPS','SANS SMALL CAPS BOLD'].map(x=>`<option value="${x}">${x}</option>`).join('')}</select></label>
    <label>Title Size<input id="m-lock-title-size" type="number" min="24" max="96" value="${Number(s.lockBrandTitleSize??85)}"></label><label>Subtitle Size<input id="m-lock-subtitle-size" type="number" min="18" max="80" value="${Number(s.lockBrandSubtitleSize??80)}"></label>
    <label>Vertical Position (293 = CENTER)<input id="m-lock-top" type="number" min="0" max="700" value="${Number(s.lockBrandTopDp??293)}"></label><label>Animation Speed ms<input id="m-lock-speed" type="number" min="280" max="3000" value="${Number(s.lockBrandAnimationSpeed??3000)}"></label>
    <label class="check"><input id="m-lock-animation" type="checkbox" ${s.lockBrandAnimationEnabled!==false?'checked':''}> Show / Animate Lock Text</label><label class="check"><input id="m-categories" type="checkbox" ${s.categoryMode?'checked':''}> Organize Launcher Into App Categories</label>
    <label class="check"><input id="m-daily-theme" type="checkbox" ${s.dailyThemeRotation?'checked':''}> Automatically Change Theme Every Day</label><label>Background Media<select id="m-media"><option value="">Launcher Default</option><option value="NONE">No Background Media</option><option value="VIDEO">Video Background</option><option value="IMAGE">Image Background</option></select></label>
  </div></section>
  ...
  $('#m-save').onclick=()=>busy($('#m-save'),async()=>{
    const settings={mode:Number($('#m-mode').value),url:$('#m-url').value.trim(),locationName:$('#m-location').value.trim(),deviceName:$('#m-device').value.trim().toUpperCase(),protectedGoogleAccount:$('#m-protected-email').value.trim().toLowerCase(),chargerMinutesPerPeso:Number($('#m-minutes').value),chargerPeso:Number($('#m-peso').value),cleanup:Number($('#m-cleanup').value),clearFacebookAfterRental:$('#m-clear-facebook').checked,clearGoogleAccountsAfterRental:$('#m-clear-google').checked,screenMonitorRemoteInterval:Number($('#m-preview').value),uiTheme:Number($('#m-theme').value),launcherBrandName:$('#m-brand').value.trim(),lockBrandSubtitle:$('#m-lock-subtitle').value.trim(),lockBrandAnimationEnabled:$('#m-lock-animation').checked,lockBrandAnimationStyle:$('#m-lock-style').value,lockBrandFontStyle:$('#m-lock-font').value,lockBrandTitleSize:Number($('#m-lock-title-size').value),lockBrandSubtitleSize:Number($('#m-lock-subtitle-size').value),lockBrandTopDp:Number($('#m-lock-top').value),lockBrandAnimationSpeed:Number($('#m-lock-speed').value),autoUpdateEnabled:$('#m-auto-update').checked,autoUpdateLockHours:Number($('#m-auto-update-hours').value),autoUpdateHour:Number($('#m-auto-update-hour').value),autoRestartEnabled:$('#m-auto-restart').checked,autoRestartHour:Number($('#m-auto-restart-hour').value),categoryMode:$('#m-categories').checked,dailyThemeRotation:$('#m-daily-theme').checked,media:$('#m-media').value,timeWidget:$('#m-widget').checked,screenMonitoringEnabled:$('#m-monitor').checked,voiceCoinEnabled:$('#m-voice-coin').checked,voiceFiveEnabled:$('#m-voice-five').checked,voiceOneEnabled:$('#m-voice-one').checked,voiceUpEnabled:$('#m-voice-up').checked,blockAdultSites:$('#m-adult').checked,blockAdDomains:$('#m-ads').checked,vpnControlsEnabled:$('#m-vpn').checked};
