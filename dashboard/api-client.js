...
const CoinTabApi=(()=>{
  const base=(window.CointabApiConfig?.baseUrl||'https://api.rentaride.top/api/v1').replace(/\/$/,'');
  let token=localStorage.getItem('cointab_api_token')||'';
  let refreshToken=localStorage.getItem('cointab_refresh_token')||'';
  let refreshing=null;
  const delay=ms=>new Promise(resolve=>setTimeout(resolve,ms));
  function saveSession(data){token=data.token||'';refreshToken=data.refreshToken||refreshToken;if(token)localStorage.setItem('cointab_api_token',token);if(refreshToken)localStorage.setItem('cointab_refresh_token',refreshToken);}
  ...
  async function raw(path,payload={},authenticated=true,attempt=0){
    ...
    try{
      const response=await fetch(base+path,{method:'POST',headers:{'Content-Type':'application/json',...(authenticated&&token?{Authorization:'Bearer '+token}:{})},body:JSON.stringify(payload),signal:controller.signal});
    ...
    }catch(error){if(attempt<2&&(error.name==='AbortError'||error.status>=500||!error.status)){await delay(500*(attempt+1));return raw(path,payload,authenticated,attempt+1);}throw error;}
    finally{clearTimeout(timeout);}
  }
  return{
    base,hasSession:()=>!!token,logout,
