...
function renderDashboard(data){state.tablets=(data.tablets||[]).filter(t=>String(t.licenseStatus||'USED').toUpperCase()==='USED'&&t.licenseKey&&t.installId).map(t=>({...t,location:tabletLocation(t)}));applyAccountDashboardTheme(state.tablets);const online=state.tablets.filter(x=>x.online).length,
