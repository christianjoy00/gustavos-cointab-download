...
function settingsModal(id){
  ...
  <section class="admin-section"><h3>Appearance & Themes</h3><p>Configure tablet branding and lock-screen settings remotely.</p><div class="form-grid admin-grid">
    ...
    <label>Launcher Theme<select id="m-theme">${themes.map((name,i)=>`<option value="${i}">${name}</option>`).join('')}</select></label><label>Animation Style<select id="m-lock-style">${['3D GLOW + PULSE','NEON FLASH','ARCADE FLASH','BOUNCE','WAVE'].map(x=>`<option value="${x}">${x}</option>`).join('')}</select></label>
    <label>Title Size<input id="m-lock-title-size" type="number" min="24" max="96" value="${Number(s.lockBrandTitleSize??72)}"></label><label>Subtitle Size<input id="m-lock-subtitle-size" type="number" min="18" max="80" value="${Number(s.lockBrandSubtitleSize??60)}"></label>
    <label>Vertical Position<input id="m-lock-top" type="number" min="0" max="700" value="${Number(s.lockBrandTopDp??293)}"></label><label>Animation Speed ms<input id="m-lock-speed" type="number" min="280" max="3000" value="${Number(s.lockBrandAnimationSpeed??900)}"></label>
  ...
  $('#m-mode').value=String(s.mode??t.mode??0);$('#m-theme').value=String(s.uiTheme??3);$('#m-media').value=String(s.media||'');$('#m-lock-style').value=String(s.lockBrandAnimationStyle||'3D GLOW + PULSE');
